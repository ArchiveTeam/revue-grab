revue-grab
=============
This is a script you can run to grab the Revue site.


More information about the archiving project can be found on the ArchiveTeam wiki: [Revue](https://wiki.archiveteam.org/index.php/Revue)
...
Follow the [instructions on the ArchiveTeam wiki](http://archiveteam.org/index.php?title=Warrior) for installing the Warrior, and select the "Revue" project in the Warrior interface.
...
Package `libzstd-dev` version 1.4.4 is required which is currently available from `buster-backports`.
    ...
    python3 -m pip install setuptools wheel
    python3 -m pip install --upgrade seesaw zstandard requests
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/revue-grab.git; cd revue-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/revue-grab/; run-pipeline3 pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, revue-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/revue-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
### For Alpine Linux:
    ...
    git clone https://github.com/ArchiveTeam/revue-grab
    cd revue-grab; ./get-wget-lua.sh
